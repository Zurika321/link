...
(function () {
  ...
  function toast(msg, type, ms) {
    ...
    el.innerHTML =
      '<i class="fa-solid ' +
      icon +
      '"></i><span></span><button class="toast-close" aria-label="Đóng"><i class="fa-solid fa-xmark"></i></button>';
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Escape")
      qsa(".modal-backdrop.open").forEach(function (bd) {
        closeModal(bd.id);
      });
  });
  ...
  function applyAccent(hex, persist) {
    ...
    qsa(".swatch").forEach(function (s) {
      s.classList.toggle(
        "active",
        s.dataset.color && s.dataset.color.toLowerCase() === hex.toLowerCase(),
      );
    });
    if (persist) {
  ...
  function buildAccentSwatches() {
    ...
    custom.title = "Tuỳ chỉnh";
    custom.innerHTML = '<i class="fa-solid fa-eye-dropper"></i>';
    var inp = document.createElement("input");
    inp.type = "color";
    inp.value = db.settings.accent || "#E8952F";
    inp.addEventListener("input", function () {
      applyAccent(inp.value, true);
    });
  ...
  function setSidebarHidden(hidden, persist) {
    ...
    if (persist) {
      db.settings.sidebarCollapsed = hidden;
      Z.save(db);
    }
  }
  $("btnCollapseSidebar").addEventListener("click", function () {
    setSidebarHidden(!sidebar.classList.contains("sidebar-hidden"), true);
  });
  $("btnMobileSidebar").addEventListener("click", function () {
    setSidebarHidden(
      sidebar.classList.contains("sidebar-hidden") ? false : true,
      false,
    );
  });
  ...
  function goRoute(route) {
    state.route = route;
    qsa(".nav-item[data-route]").forEach(function (b) {
      b.classList.toggle("active", b.dataset.route === route);
    });
    ["dashboard", "links", "categories", "tags", "data", "settings"].forEach(
      function (r) {
  ...
  qsa(".nav-item[data-route]").forEach(function (b) {
    b.addEventListener("click", function () {
      goRoute(b.dataset.route);
    });
  });

  /* ================= helpers dùng chung ================= */
  function categoryOf(id) {
    return db.categories.find(function (c) {
      return c.id === id;
    });
  }
  ...
  function miniIcon(link, cat) {
    ...
    if (link.iconClass) {
      ...
      ci.className = link.iconClass;
      ci.style.color = link.iconColor || (cat && cat.color) || "var(--accent)";
      ci.style.fontSize = "14px";
      wrap.appendChild(ci);
      return wrap;
    }
    ...
    if (src) {
      ...
      img.onerror = function () {
        img.remove();
        wrap.appendChild(catIconEl(cat));
      };
      wrap.appendChild(img);
    } else wrap.appendChild(catIconEl(cat));
    return wrap;
  }
  function parseIconInput(value, oldIconClass, oldIconColor) {
    value = (value || "").trim();
    if (!value) return { icon: "", iconClass: "", iconColor: "" };
    var looksLikeClass =
      /^(fa-|fa[bsrl]\s)/i.test(value) ||
      (!/^https?:\/\//i.test(value) && /\bfa-/.test(value));
    if (looksLikeClass) {
      var sameAsBefore = value === oldIconClass;
      return {
        icon: "",
        iconClass: value,
        iconColor: sameAsBefore ? oldIconColor || "" : "",
      };
    }
    return { icon: value, iconClass: "", iconColor: "" };
  }
  function catIconEl(cat) {
    var i = document.createElement("i");
    i.className = (cat && cat.icon) || "fa-solid fa-link";
    i.style.color = (cat && cat.color) || "var(--muted)";
  ...
  function renderDashboard() {
    var all = db.links;
    var starred = all.filter(function (l) {
      return l.starred;
    }).length;
    var cards = [
    ...
    db.categories.forEach(function (cat) {
      var count = all.filter(function (l) {
        return l.categoryId === cat.id;
      }).length;
      var row = document.createElement("div");
      row.className = "bar-row";
      row.innerHTML =
        '<div class="bar-label"><i class="' +
        cat.icon +
        '" style="color:' +
        cat.color +
        '"></i>' +
    ...
    var recentWrap = $("recentList");
    recentWrap.innerHTML = "";
    var recents = all
      .filter(function (l) {
        return l.lastViewedAt;
      })
      .sort(function (a, b) {
    ...
    if (!recents.length)
      recentWrap.innerHTML =
        '<p style="font-size:12.5px;color:var(--muted)">Chưa có lượt xem nào.</p>';
    recents.forEach(function (l) {
      ...
      ico.appendChild(miniIcon(l, cat));
      row.appendChild(ico);
      ...
      time.textContent = Z.formatRelativeTime(l.lastViewedAt);
    ...
    var remWrap = $("dashReminders");
    remWrap.innerHTML = "";
    var due = Z.dueReminders(db);
    if (!due.length)
      remWrap.innerHTML =
        '<p style="font-size:12.5px;color:var(--muted)">Không có lịch nào đến hạn trong 24h qua.</p>';
    due.forEach(function (d) {
      var row = document.createElement("div");
      row.className = "recent-row";
      row.innerHTML =
        '<i class="fa-solid fa-bell" style="color:var(--accent);width:28px;text-align:center"></i>' +
        '<div class="recent-title">' +
        d.link.title +
        '</div><div class="recent-time">' +
        d.status.dueAgoLabel +
        "</div>";
  ...
  function fillCategorySelect(selectEl, selectedId, withAllOption) {
    ...
    if (withAllOption) {
      ...
      oa.value = "all";
      oa.textContent = "Tất cả danh mục";
  ...
  function fillTagFilterSelect() {
    ...
    sel.value =
      cur &&
      qsa("option", sel).some(function (o) {
        return o.value === cur;
      })
        ? cur
        : "all";
  ...
  function getFilteredAdminLinks() {
    var list = db.links.filter(function (l) {
      if (state.linkCategory !== "all" && l.categoryId !== state.linkCategory)
        return false;
      if (
        state.linkTag !== "all" &&
        (l.tags || []).indexOf(state.linkTag) === -1
      )
        return false;
      if (state.linkSearch) {
        var hay = Z.normalizeSearch(
          [l.title, l.desc, (l.tags || []).join(" "), l.url].join(" "),
        );
  ...
  function renderLinksTable() {
    ...
    var tbody = $("linksTableBody");
    tbody.innerHTML = "";
    if (!pageItems.length) {
      tbody.innerHTML =
        '<tr><td colspan="8" style="text-align:center;color:var(--muted);padding:30px">Không có liên kết nào khớp bộ lọc.</td></tr>';
    }
    pageItems.forEach(function (l) {
      ...
      cb.type = "checkbox";
      cb.checked = state.selected.has(l.id);
      cb.addEventListener("change", function () {
        toggleSelect(l.id, cb.checked);
      });
      ...
      tdLink.appendChild(cell);
      tr.appendChild(tdLink);

      var tdCat = document.createElement("td");
      tdCat.innerHTML =
        '<span class="cat-chip" style="background:rgba(0,0,0,.06)"><span class="dot"></span>' +
        (cat.name || "—") +
        "</span>";
      tdCat.querySelector(".cat-chip").style.background = cat.color
        ? "color-mix(in srgb, " + cat.color + " 16%, transparent)"
        : "";
      tdCat.querySelector(".dot").style.background =
        cat.color || "var(--muted)";
      ...
      editBtn.className = "btn btn-icon btn-sm btn-ghost";
      editBtn.innerHTML = '<i class="fa-solid fa-pen"></i>';
      editBtn.addEventListener("click", function () {
        openLinkModal(l);
      });
      var delBtn = document.createElement("button");
      delBtn.className = "btn btn-icon btn-sm btn-ghost";
      delBtn.innerHTML =
        '<i class="fa-solid fa-trash" style="color:var(--danger)"></i>';
      delBtn.addEventListener("click", function () {
        openConfirm(
          "Xoá liên kết?",
          'Xoá "' + l.title + '" khỏi danh sách.',
          function () {
            Z.deleteLink(db, l.id);
            Z.save(db);
    ...
    if (totalPages > 1) {
      ...
      prev.className = "btn btn-icon btn-sm btn-ghost";
      ...
      next.className = "btn btn-icon btn-sm btn-ghost";
  ...
  function updateCheckAllState(pageItems) {
    var all =
      pageItems.length > 0 &&
      pageItems.every(function (l) {
        return state.selected.has(l.id);
      });
    $("checkAll").checked = all;
  }
  $("checkAll").addEventListener("change", function () {
    var list = getFilteredAdminLinks().slice(
      (state.page - 1) * state.pageSize,
      state.page * state.pageSize,
    );
    list.forEach(function (l) {
      toggleSelect(l.id, $("checkAll").checked);
    });
    renderLinksTable();
  });
  ...
  $("btnBulkMove").addEventListener("click", function () {
    ...
    state.selected.forEach(function (id) {
      Z.updateLink(db, id, { categoryId: catId });
    });
    Z.save(db);
    renderLinksTable();
    toast(
      "Đã chuyển danh mục cho " + state.selected.size + " liên kết",
      "success",
    );
  });

  $("linkSearch").addEventListener("input", function () {
    state.linkSearch = Z.normalizeSearch(this.value);
    state.page = 1;
    renderLinksTable();
  });
  $("filterCategory").addEventListener("change", function () {
    state.linkCategory = this.value;
    state.page = 1;
    renderLinksTable();
  });
  $("filterTag").addEventListener("change", function () {
    state.linkTag = this.value;
    state.page = 1;
    renderLinksTable();
  });
  $("linkSort").addEventListener("change", function () {
    state.linkSort = this.value;
    renderLinksTable();
  });
  $("btnAddLinkAdmin").addEventListener("click", function () {
    openLinkModal(null);
  });

  /* ================= LINK MODAL (thêm/sửa) ================= */
  function fillTimeSelects() {
    var dayEl = $("fReminderDay");
    dayEl.innerHTML = "";
    ...
    var hourEl = $("fReminderHour");
    hourEl.innerHTML = "";
    for (var h = 0; h < 24; h++) {
      var o1 = document.createElement("option");
      o1.value = h;
      o1.textContent = h + " giờ";
      hourEl.appendChild(o1);
    }
    var minEl = $("fReminderMinute");
    minEl.innerHTML = "";
    for (var m = 0; m < 60; m++) {
      var o2 = document.createElement("option");
      o2.value = m;
      o2.textContent = m + " phút";
      minEl.appendChild(o2);
    }
  }
  fillTimeSelects();
  ...
  function renderTagChips() {
    var wrap = $("tagWrap");
    qsa(".tag-pill", wrap).forEach(function (p) {
      p.remove();
    });
    state.tagsDraft.forEach(function (t) {
      ...
      pill.innerHTML =
        "#" +
        t +
        ' <button type="button" aria-label="Xoá thẻ">&times;</button>';
      pill.querySelector("button").addEventListener("click", function () {
        state.tagsDraft = state.tagsDraft.filter(function (x) {
          return x !== t;
        });
        renderTagChips();
      });
  ...
  $("fTagInput").addEventListener("keydown", function (e) {
    if (e.key === "Enter" || e.key === ",") {
      ...
      if (v && state.tagsDraft.indexOf(v) === -1) {
        state.tagsDraft.push(v);
        renderTagChips();
      }
      e.target.value = "";
    } else if (
      e.key === "Backspace" &&
      !e.target.value &&
      state.tagsDraft.length
    ) {
      state.tagsDraft.pop();
      renderTagChips();
    }
  });
  $("fReminderOn").addEventListener("change", function () {
    $("reminderFields").hidden = !this.checked;
  });

  function openLinkModal(link) {
    state.editingId = link ? link.id : null;
    $("linkModalTitle").textContent = link
      ? "Chỉnh sửa liên kết"
      : "Thêm liên kết";
    $("fUrl").value = link ? link.url : "";
    $("fTitle").value = link ? link.title : "";
    $("fDesc").value = link ? link.desc : "";
    $("fIcon").value = link ? link.iconClass || link.icon || "" : "";
    ...
    if (hasReminder) {
      ...
      $("fReminderDay").value = 1;
      $("fReminderHour").value = 20;
      $("fReminderMinute").value = 0;
    }
  ...
  $("btnSaveLink").addEventListener("click", function () {
    ...
    var existingLink = state.editingId
      ? db.links.find(function (l) {
          return l.id === state.editingId;
        })
      : null;
    var iconParsed = parseIconInput(
      $("fIcon").value,
      existingLink && existingLink.iconClass,
      existingLink && existingLink.iconColor,
    );
    var data = {
      url: url,
      title: $("fTitle").value.trim() || url,
      desc: $("fDesc").value.trim(),
      categoryId: $("fCategory").value,
      tags: state.tagsDraft.slice(),
      icon: iconParsed.icon,
      iconClass: iconParsed.iconClass,
      iconColor: iconParsed.iconColor,
      starred: $("fStarred").checked,
      reminder: $("fReminderOn").checked
        ? {
            weekday: parseInt($("fReminderDay").value, 10),
            hour: parseInt($("fReminderHour").value, 10),
            minute: parseInt($("fReminderMinute").value, 10),
          }
        : null,
    };
    if (state.editingId) {
      Z.updateLink(db, state.editingId, data);
      toast("Đã lưu thay đổi", "success");
    } else {
      Z.addLink(db, data);
      toast("Đã thêm liên kết", "success");
    }
    Z.save(db);
  ...
  function renderCategoriesRoute() {
    ...
    db.categories.forEach(function (cat, idx) {
      var count = db.links.filter(function (l) {
        return l.categoryId === cat.id;
      }).length;
      var row = document.createElement("div");
      ...
      order.className = "cat-order-btns";
      ...
      down.innerHTML = "▼";
      down.disabled = idx === db.categories.length - 1;
      ...
      colorBox.className = "cm-color";
      colorBox.style.background = cat.color;
      colorBox.innerHTML = '<i class="' + cat.icon + '"></i>';
      ...
      info.querySelector(".cm-sub").textContent =
        count + " liên kết" + (cat.protected ? " · bảo vệ" : "");
      row.appendChild(info);

      if (cat.protected) {
        var tag = document.createElement("span");
        tag.className = "protected-tag";
        tag.textContent = "hệ thống";
        row.appendChild(tag);
      }
      ...
      renameBtn.className = "btn btn-icon btn-sm btn-ghost";
      renameBtn.innerHTML = '<i class="fa-solid fa-pen"></i>';
      renameBtn.addEventListener("click", function () {
        renameCategoryPrompt(cat);
      });
      ...
      if (!cat.protected) {
        ...
        delBtn.innerHTML =
          '<i class="fa-solid fa-trash" style="color:var(--danger)"></i>';
        delBtn.title = count
          ? "Danh mục còn " + count + " liên kết, hãy chuyển hết trước khi xoá"
          : "Xoá danh mục";
        delBtn.addEventListener("click", function () {
          ...
          if (!res.ok) {
            toast(
              res.reason === "in-use"
                ? "Danh mục còn liên kết, hãy chuyển hết trước khi xoá."
  ...
  function renameCategoryPrompt(cat) {
    ...
    var icon =
      window.prompt(
        "Class icon Font Awesome (vd: fa-solid fa-star):",
        cat.icon,
      ) || cat.icon;
  ...
  $("btnAddCategory").addEventListener("click", function () {
    ...
    if (!name) {
      toast("Nhập tên danh mục trước đã", "error");
      return;
    }
    Z.addCategory(db, {
      name: name,
      icon: $("newCatIcon").value.trim() || "fa-solid fa-tag",
      color: $("newCatColor").value,
    });
    Z.save(db);
    $("newCatName").value = "";
    $("newCatIcon").value = "fa-solid fa-tag";
    renderCategoriesRoute();
  ...
  function renderTagsRoute() {
    ...
    if (!tags.length) {
      wrap.innerHTML =
        '<p style="padding:20px;color:var(--muted);font-size:13px">Chưa có thẻ nào — thêm thẻ khi tạo/sửa liên kết.</p>';
      return;
    }
    wrap.innerHTML = "";
    tags.forEach(function (t) {
      ...
      input.addEventListener("change", function () {
        ...
        if (v && v !== t.name) {
          Z.renameTag(db, t.name, v);
          Z.save(db);
          renderTagsRoute();
          toast("Đã đổi tên thẻ", "success");
        }
      });
      ...
      del.addEventListener("click", function () {
        openConfirm(
          'Xoá thẻ "#' + t.name + '"?',
          "Thẻ sẽ được gỡ khỏi " + t.count + " liên kết.",
          function () {
            Z.deleteTag(db, t.name);
            Z.save(db);
            renderTagsRoute();
            toast("Đã xoá thẻ", "success");
          },
        );
      });
      row.appendChild(del);
  ...
  function renderDataRoute() {
    ...
    $("storageSizeInfo").textContent =
      "Dung lượng dữ liệu hiện tại: ~" + (bytes / 1024).toFixed(1) + " KB";
  }
  $("btnExportJson").addEventListener("click", function () {
    Z.exportJSON(db);
    toast("Đã tải file JSON", "success");
  });
    ...
    if (res.ok) {
      ...
      toast(
        mode === "replace"
          ...
          : "Đã gộp dữ liệu (" + (res.added || 0) + " link mới)",
        "success",
      );
  ...
  $("btnResetAll").addEventListener("click", function () {
    openConfirm(
      "Xoá toàn bộ dữ liệu?",
      "Không thể hoàn tác. Hãy chắc chắn bạn đã xuất file JSON sao lưu.",
      function () {
        db = Z.resetAll();
        goRoute("dashboard");
  ...
  $("toggleMouse").addEventListener("click", function () {
    db.settings.virtualMouse = !db.settings.virtualMouse;
    Z.save(db);
    syncToggle(this, db.settings.virtualMouse);
  });
  $("toggleHidden").addEventListener("click", function () {
    db.settings.showHidden = !db.settings.showHidden;
    Z.save(db);
  ...
  $("btnBgApply").addEventListener("click", function () {
    db.settings.background = $("bgInput").value.trim();
    Z.save(db);
    toast("Đã áp dụng — xem tại trang chính", "success");
  });
  $("btnBgClear").addEventListener("click", function () {
    $("bgInput").value = "";
    db.settings.background = "";
    Z.save(db);
  });

  function refreshLockStatus() {
    $("lockStatusText").textContent = db.settings.passwordHash
      ? "Đã bật — bấm để đổi mật khẩu"
      : "Chưa đặt mật khẩu";
  ...
  $("toggleLock").addEventListener("click", function () {
    if (db.settings.passwordHash) {
      openConfirm(
        "Tắt khoá màn hình?",
        "Trang chính sẽ không còn yêu cầu mật khẩu.",
        function () {
          db.settings.passwordHash = null;
          Z.save(db);
          refreshLockStatus();
  ...
  $("btnSavePass").addEventListener("click", function () {
    var p1 = $("fPass1").value,
      p2 = $("fPass2").value;
    if (p1.length < 4) {
      toast("Mật khẩu cần ít nhất 4 ký tự", "error");
      return;
    }
    if (p1 !== p2) {
    ...
    db.settings.passwordHash = Z.hashPass(p1);
    Z.save(db);
    closeModal("passModalBackdrop");
    refreshLockStatus();
  ...
  function boot() {
    ...
    refreshLockStatus();
    setSidebarHidden(
      window.innerWidth < 900 ? true : !!db.settings.sidebarCollapsed,
      false,
    );
