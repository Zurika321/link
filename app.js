...
(function () {
  ...
  function visibleLinks() {
    return db.links.filter(function (l) {
      return db.settings.showHidden || l.categoryId !== "zz";
    });
  }
  function renderStatPills() {
    ...
    var recent = all.filter(function (l) {
    ...
    defs.forEach(function (d) {
      var b = document.createElement("button");
      b.className =
        "stat-pill" + (state.filter.view === d.view ? " active" : "");
      b.innerHTML =
        '<i class="fa-solid ' +
        d.icon +
        '"></i><span>' +
  ...
  function renderCategoryRail() {
    ...
    db.categories.forEach(function (cat) {
      ...
      var b = document.createElement("button");
      b.className =
        "cat-tab" + (state.filter.categoryId === cat.id ? " active" : "");
      b.style.setProperty("--dot-color", cat.color);
      b.innerHTML =
        '<span class="dot"></span><i class="' +
        cat.icon +
        '"></i><span>' +
        cat.name +
        '</span><span class="count mono">' +
        count +
        "</span>";
      b.addEventListener("click", function () {
        state.filter.categoryId =
          state.filter.categoryId === cat.id ? "all" : cat.id;
        render();
      });
      wrap.appendChild(b);
    });
  ...
  function getFilteredLinks() {
    ...
    // ghim link đã đánh dấu lên đầu — sort ổn định (stable) nên thứ tự bên trong
    // từng nhóm (đã đánh dấu / chưa đánh dấu) vẫn giữ nguyên theo sort ở trên
    list.sort(function (a, b) {
  ...
  function buildCard(link, index) {
    var cat = categoryOf(link.categoryId) || {
      color: "var(--accent)",
    ...
    iconWrap.className = "lc-icon";
    if (link.iconClass) {
      iconWrap.appendChild(customIconEl(link, cat));
    } else {
    ...
    // tên rút gọn hiện ở dạng lưới icon-only (compact) — chi tiết đầy đủ nằm trong popover bên dưới
    var compactName = document.createElement("div");
    compactName.className = "lc-compact-name";
    compactName.textContent = link.title;

    var popover = document.createElement("div");
    popover.className = "lc-popover";
    popover.appendChild(title);
    popover.appendChild(desc);
    if ((link.tags || []).length) popover.appendChild(tagsWrap);
    popover.appendChild(meta);

    card.appendChild(top);
    card.appendChild(compactName);
  ...
  function customIconEl(link, cat) {
    var i = document.createElement("i");
    i.className = link.iconClass;
    i.style.color = link.iconColor || (cat && cat.color) || "var(--accent)";
  ...
  function renderGrid() {
    ...
    grid.classList.toggle("compact-mode", state.viewMode === "compact");
    ...
    list.forEach(function (link, i) {
      frag.appendChild(buildCard(link, i));
    });
    ...
    if (!state.search) {
      ...
      addCard.innerHTML =
        '<i class="fa-solid fa-plus"></i><span>Thêm link</span>';
  ...
  function openCtxMenu(linkId, x, y) {
    ...
    menu.innerHTML =
      ...
      '<button data-act="star" class="' +
      (link.starred ? "ctx-active" : "") +
      '"><i class="fa-solid fa-star"></i> ' +
      (link.starred ? "Bỏ đánh dấu" : "Đánh dấu") +
      "</button>" +
    ...
    qsa("button", menu).forEach(function (btn) {
      btn.addEventListener("click", function () {
        handleCtxAction(btn.dataset.act, link);
      });
    });
  ...
  function handleCtxAction(act, link) {
    ...
    if (act === "edit") openLinkModal(link);
      ...
      navigator.clipboard
        .writeText(link.url)
        .then(function () {
          toast("Đã copy liên kết", "success");
        })
        .catch(function () {
          toast("Không thể copy liên kết", "error");
        });
    } else if (act === "open") {
      openLink(link);
    } else if (act === "delete") {
  ...
  $("btnSaveLink").addEventListener("click", function () {
    ...
    if (!url || (!isFile && !Z.isValidURL(url))) {
      toast(
        "Đường dẫn không hợp lệ. URL cần bắt đầu bằng http(s):// hoặc file:///",
        "error",
      );
      return;
    }
  ...
  qsa("#linkForm .field input, #linkForm .field textarea").forEach(
    function (inp) {
      inp.addEventListener("input", function () {
        inp.closest(".field").classList.toggle("has-value", !!inp.value);
      });
    },
  );
  ...
  $("viewGrid").addEventListener("click", function () {
    ...
    db.settings.viewMode = "grid";
    Z.save(db);
    renderGrid();
  });
  $("viewCompact").addEventListener("click", function () {
    state.viewMode = "compact";
    setActiveViewBtn("compact");
    db.settings.viewMode = "compact";
    Z.save(db);
    renderGrid();
  });
  ...
  $("toggleLock").addEventListener("click", function () {
    if (db.settings.passwordHash) {
      openConfirm(
        "Tắt khoá màn hình?",
        "Bạn sẽ không cần nhập mật khẩu để mở trang nữa.",
        function () {
  ...
  $("btnImportConfirm").addEventListener("click", function () {
    ...
    if (res.ok) {
      ...
      toast(res.error || "Nhập dữ liệu thất bại", "error");
    }
  });
  ...
  function renderReminderBar() {
    ...
    due.forEach(function (d) {
      ...
      item.innerHTML =
        '<i class="fa-solid fa-bell"></i><span class="rt-name"></span><span class="rt-time mono"></span>';
  ...
  function reminderNotifyTick() {
    ...
    Z.dueReminders(db).forEach(function (d) {
      if (d.status.diffMs < 60000) {
        var key =
          d.link.id + "-" + Math.floor(Date.now() / (7 * 24 * 3600 * 1000));
        if (!notifiedBucket[key]) {
          notifiedBucket[key] = true;
          toast(
            '🔔 "' + d.link.title + '" vừa đến lịch cập nhật mới!',
            "info",
            6000,
          );
        }
  ...
  /* ---- quản lý lịch nhắc ---- */
  $("btnReminderManage").addEventListener("click", function () {
    renderReminderManager();
    openModal("reminderModalBackdrop");
  });
  function renderReminderManager() {
    var wrap = $("reminderManageList");
    wrap.innerHTML = "";
    var withRem = db.links.filter(function (l) {
      return l.reminder;
    });
    var without = db.links.filter(function (l) {
      return !l.reminder;
    });

    var h1 = document.createElement("div");
    h1.className = "section-title";
    h1.textContent = "Đang bật (" + withRem.length + ")";
    wrap.appendChild(h1);
    if (!withRem.length) {
      var p = document.createElement("p");
      p.style.cssText =
        "font-size:12.5px;color:var(--muted);margin-bottom:16px";
      p.textContent = "Chưa có link nào đặt lịch.";
      wrap.appendChild(p);
    }
    withRem.forEach(function (l) {
      ...
      row.innerHTML =
        ...
        Z.WEEKDAY_LABELS[l.reminder.weekday] +
        ", " +
        String(l.reminder.hour).padStart(2, "0") +
        ":" +
        String(l.reminder.minute).padStart(2, "0") +
        "</div></div>" +
        '<button class="btn btn-icon btn-sm btn-ghost" data-remove><i class="fa-solid fa-trash"></i></button>';
      row.querySelector("[data-remove]").addEventListener("click", function () {
        Z.updateLink(db, l.id, { reminder: null });
        Z.save(db);
        renderReminderManager();
        renderReminderBar();
    ...
    h2.textContent = "Thêm lịch nhắc mới";
    wrap.appendChild(h2);
    if (!without.length) {
      wrap.appendChild(
        Object.assign(document.createElement("p"), {
          textContent: "Tất cả link đã có lịch.",
    ...
    sel.className = "select-plain";
    sel.style.flex = "1";
    without.forEach(function (l) {
      var o = document.createElement("option");
      o.value = l.id;
    ...
    pickRow.appendChild(dSel);
    pickRow.appendChild(hSel);
    pickRow.appendChild(mSel);
    pickRow.appendChild(addBtn);
    wrap.appendChild(pickRow);

    addBtn.addEventListener("click", function () {
      Z.updateLink(db, sel.value, {
        reminder: {
          weekday: parseInt(dSel.value, 10),
          hour: parseInt(hSel.value, 10),
          minute: parseInt(mSel.value, 10),
        },
      });
      Z.save(db);
      renderReminderManager();
  ...
  function placeVMouseDefault() {
    vMouse.style.left = "16px";
    vMouse.style.top = "78px";
  }
  vMouse.addEventListener("pointerdown", function (e) {
    vDragging = true;
    vMouse.setPointerCapture(e.pointerId);
  ...
  vMouse.addEventListener("pointermove", function (e) {
    ...
    qsa(".link-card.is-hovered").forEach(function (c) {
    ...
    if (card) {
      ...
      vTargetLink = db.links.find(function (l) {
        return l.id === id;
      });
  ...
  vMouse.addEventListener("pointerup", function (e) {
    vDragging = false;
    vMouse.releasePointerCapture(e.pointerId);
  });
  goBtn.addEventListener("click", function () {
    if (vTargetLink) openLink(vTargetLink);
  });
  ...
  function boot() {
    ...
    refreshLockStatus();
    checkLockOnBoot();
    state.viewMode = db.settings.viewMode || "grid";
    setActiveViewBtn(state.viewMode);
    render();
    renderReminderBar();
  }
  boot();
