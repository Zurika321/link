...
function create_icon(title) {
  ...
  if (title == "All") {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (index !== -1) {
        ...
        a.className = "link-with-favicon";
        a.textContent = items[i]; // Thêm nội dung liên kết (tùy chọn)
  ...
  button.classList = "button_add";
  button.textContent = "+";
  button.onclick = function () {
    close();
  };
  div_link.appendChild(button);
  div_scroll.appendChild(div_link);
}
create_icon("All");
var ZZ_on = false;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  divScroll.addEventListener("mouseover", function (event) {
    if (event.target.classList.contains("link-with-favicon")) {
  ...
  searchInput.addEventListener("input", function () {
    ...
    divLinks.sort((a, b) => {
      ...
      return bSimilarity - aSimilarity || aValue.localeCompare(bValue);
    });
