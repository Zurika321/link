...
!(function () {
  ...
  function linkss() {
    ...
    links.forEach(async (link) => {
      ...
      try {
        ...
        for (const { key, value } of faviconMappings) {
          if (key(url)) {
            faviconUrl = typeof value === "function" ? await value(url) : value;
            break;
          }
        }
  ...
  const menu_animetody = document.getElementById("footer");

  menu_content.forEach(enableScroll);
  enableScroll(divMain);
  enableScroll(list_title);
  enableScroll(menu_animetody);
  ...
  function update_date_anime() {
    ...
    if (note.length !== 0) {
      ...
      note_anime.map((element) => {
        ...
        if (match) {
          ...
          if (
            ...
            if (
              currentTotalMinutes <=
              nextDayTotalMinutes - convertToMinutes(0, hour, minute)
            ) {
              today.push(`hôm nay (khoản ${hours}h${minutes}m trước)`);
            } else {
      ...
      let list_anime = [];
      for (let index = 0; index < note_anime2.length; index++) {
        list_anime.push({
          Name: note_anime2[index],
          time: time[index],
          today: today[index],
        });
      }
      thong_bao_anime(list_anime, note_anime2);
    }
  }
  setInterval(update_date_anime, 60000);
  function thong_bao_anime(list_anime, note_anime2) {
    const div_thong_bao = document.querySelectorAll(".thong_bao");
    let list_anime_loc = [];
    div_thong_bao.forEach(function (div) {
      ...
      if (index_thong_bao !== -1) {
        ...
        list_anime_loc.push(list_anime[index_thong_bao]);
      } else {
        div.style.display = "none";
      }
    });
    if (list_anime_loc.length == 0) {
      list_anime_loc.push({
        Name: "<br>...",
        time: ["Không có anime nào vừa ra tập mới"],
        today: "...",
      });
    }
    create_animetoday(list_anime_loc);
  }

  function create_animetoday(list_anime) {
    let menu_animetody = document.getElementById("list_anime");
    let list = "";
    for (let i = 0; i < list_anime.length; i++) {
      list += `<div class="anime"> ${
        list_anime[i].Name.split("<br>")[1]
      } <br/> ${list_anime[i].time[0]} <br/> ${list_anime[i].today}</div>`;
    }
    menu_animetody.innerHTML = list;
  }
  ...
  function create_icon() {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (index !== -1) {
        ...
        const itemText = items[i].split("<br>")[1] + items[i].split("<br>")[2];
        const titleParts = itemText.split("</i>");
        const title_div_link =
          titleParts.length > 1 ? titleParts[1] : titleParts[0];
        div_link.setAttribute("title", title_div_link.trim());
    ...
    div_link.setAttribute("title", "Add link");
  ...
  document
    ...
    .addEventListener("click", edit_data_anime);
  function edit_data_anime() {
    document.getElementById("class_box2_settinganime").style.display = "block";
    let note_anime = note.filter((note) => note.startsWith("Anime"));
    let option = "";
    for (let index = 0; index < note_anime.length; index++) {
      option += `<option value="${note_anime[index]}">${
        note_anime[index].split("<br>")[1]
      }</option>`;
    }
    document.getElementById("select_anime").innerHTML = option;
    document.getElementById("select_anime").value = note_anime[0];
    let edit_note = note_anime[0];

    document
      .getElementById("select_anime")
      .addEventListener("change", function (event) {
        const selectedValue = event.target.value;
        edit_note = selectedValue;
        if (
          edit_note.split("<br>")[2] !== "" &&
          edit_note.split("<br>")[2].includes("-") && // Kiểm tra có chứa dấu "-"
          edit_note.split("<br>")[2].split("(")[1] != null &&
          edit_note.split("<br>")[2].split("(")[1].split(" ")[1].split("-")
            .length === 3 // Kiểm tra có đúng 3 phần sau khi tách bằng dấu "-"
        ) {
          const dayValue = parseInt(
            edit_note.split("<br>")[2].split("(")[1].split(" ")[1].split("-")[0]
          );
          if (isNaN(dayValue) || dayValue < 2 || dayValue > 8) {
            document.getElementById("d").innerText = 0;
            document.getElementById("h").innerText = 0;
            document.getElementById("m").innerText = 0;
          } else {
            document.getElementById("d").innerText = dayValue;
            document.getElementById("h").innerText = edit_note
              .split("<br>")[2]
              .split(" ")[1]
              .split("-")[1];
            document.getElementById("m").innerText = edit_note
              .split("<br>")[2]
              .split(" ")[1]
              .split("-")[2]
              .split(")")[0];
    ...
    document.getElementById("delete_anime").onclick = function () {
      let notes = edit_note.split("<br>");
      let input_2 = "(new " + 10 + "-" + 0 + "-" + 0 + ")";
      let new_note = notes[0] + "<br>" + notes[1] + "<br>" + input_2;
      let retrievedData = JSON.parse(localStorage.getItem("data"));
      ...
      console_data();
      title_pick(what_title);
      thong_bao_chung("Xóa khỏi danh sách thông báo anime thành công");
    };

    document.getElementById("save_anime").onclick = function () {
      let notes = edit_note.split("<br>");
      ...
      console_data();
      title_pick(what_title);
      thong_bao_chung("Lưu thành công");
    };

    document.getElementById("close_setting_anime").onclick = function () {
      document.getElementById("class_box2_settinganime").style.display = "none";
    };
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    divScroll.addEventListener("mouseover", function (event) {
      ...
      //   whaticon.innerHTML = `Thêm icon mới`;
      //   whaticon.style =
    ...
    //     ruler1: "Menu",
    //     ruler2: "Menu",
    //     whaticon: "Khung Name List",
    //   };
    //   if (textMap[targetId]) {
    //     whaticon.innerHTML = textMap[targetId];
    //     whaticon.style = styles;
    //   }
    // });
    ...
    function handleFile(e) {
      try {
        ...
        reader.onload = function (event) {
          ...
          console_data();
          anime_tap_moi = update_date_anime();
          title_pick("All");
  ...
  function setDivHeight() {
    ...
    divScroll.style.height = windowHeight - 215 + "px";
  ...
  document
    .getElementById("btn_link_anh_nen")
    .addEventListener("click", add_link_anh_nen, false);
  function add_link_anh_nen() {
    document.getElementById(
      "div_main"
    ).style.backgroundImage = `url('https://i.pinimg.com/originals/b6/7c/0f/b67c0fed6c939c5932bae21c0bb53245.jpg')`;
    document.getElementById("div_main").style.filter =
